...
import { ArrowRight, BookOpenCheck, CheckCircle2, FileText, Layers3 } from "lucide-react";
...
export function WorkflowGuide({ projectId, compact = false }: { projectId: string; compact?: boolean }) {
  return (
    <section className="tool-panel overflow-hidden">
      <div className="grid gap-4 border-b border-line bg-[#fbfcfa] p-4 md:grid-cols-[1fr_auto]">
      <div className="flex flex-wrap items-end justify-between gap-2">
        ...
        <div className="rounded border border-line bg-white px-3 py-2 text-xs leading-5 text-ink/60">
          草稿不改设定；接受章节才更新记忆。
        </div>
      </div>
      <div className="p-4">
      <div className={`mt-4 grid gap-3 ${compact ? "md:grid-cols-2" : "lg:grid-cols-4"}`}>
        {steps.map((step) => (
          <StepCard key={step.title} step={step} projectId={projectId} />
        ))}
      </div>
...
function StepCard({ step, projectId }: { step: { icon: LucideIcon; title: string; body: string; href: string; action: string }; projectId: string }) {
  ...
  return (
    <div className="rounded border border-line bg-white p-3">
      ...
      <Link className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-accent hover:underline" href={`/projects/${projectId}/${step.href}`}>
