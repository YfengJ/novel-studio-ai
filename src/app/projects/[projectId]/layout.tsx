...
import { BookOpen, Boxes, Database, FileJson, GitFork, Home, PenLine, Settings, Users } from "lucide-react";
...
export default async function ProjectLayout({ children, params }: { children: React.ReactNode; params: Promise<{ projectId: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-paper">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-line bg-[#fbfaf6] p-5 lg:block">
        ...
        <div className="mt-6 rounded border border-line bg-white p-4">
          <div className="text-xs uppercase tracking-[0.22em] text-ink/45">Novel Studio</div>
          <div className="mt-2 text-lg font-semibold">长篇创作工作台</div>
          <p className="mt-2 text-xs leading-5 text-ink/60">设定、大纲、正文、检查和记忆入库分步执行。</p>
        </div>
        <nav className="mt-5 grid gap-1">
          {nav.map((item) => {
            const Icon = item.icon;
            return (
              <Link key={item.href || "dashboard"} href={`/projects/${projectId}/${item.href}`} className="flex items-center gap-3 rounded border border-transparent px-3 py-2.5 text-sm text-ink/80 hover:border-line hover:bg-white hover:text-ink">
                <Icon size={16} /> {item.label}
              </Link>
            );
          })}
        </nav>
        <div className="absolute bottom-5 left-5 right-5 rounded border border-line bg-white p-3 text-xs leading-5 text-ink/60">
